...
void brute() {
	int res = 0;
	pnt maxp(0, 0), minp(0x3fffffff, 0x3fffffff);
	for (int i = 0; i < N; ++i) {
		maxp.x = max(maxp.x, p[i].x), maxp.y = max(maxp.y, p[i].y);
		minp.x = min(minp.x, p[i].x), minp.y = min(minp.y, p[i].y);
	}
	long long area = getArea(maxp, minp);
	for (int i = 0; i < (1 << N); ++i) {
		int cnt = 0;
		for (int j = 0; j < N; ++j)
			if (i & (1 << j)) cnt++;
		if (cnt != K) continue;
		pnt bp(0, 0), sp(0x3fffffff, 0x3fffffff);
		for (int j = 0; j < N; ++j)
			if (!(i & (1 << j))) {
				bp.x = max(bp.x, p[j].x), bp.y = max(bp.y, p[j].y);
				sp.x = min(sp.x, p[j].x), sp.y = min(sp.y, p[j].y);
			}
		if (getArea(bp, sp) < area || sp.x == 0x3fffffff) res++;
	}
	printf("%d\n", res);
}
void calc() {
